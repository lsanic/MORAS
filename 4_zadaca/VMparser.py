class Parser:
    ...
    def _parseLine(self, line, n):
        ...
        if l[0] == "push":
            x = l[2]
            if len(l) == 3 and int(x) >= 0 and x.isdigit():
                return "//" + " ".join(l) + "\n" + self._push(l[1], l[2], n)
            else:
                self._flag = False
                Parser._error("Parser", n, "Error:Nedefinirana naredba.");
                return ""
        elif l[0] == "pop":
            y = l[2]
            if len(l) == 3 and int(y) >= 0 and y.isdigit():
                return "//" + " ".join(l) + "\n" + self._pop(l[1], l[2], n)
            else:
                self._flag = False
                Parser._error("Parser", n, "Error:Nedefinirana naredba.");
    ...
    def _ifgoto(self, lab, n):
        if lab in self._labels:
           return "@SP\nAM=M-1\nD=M+1\n@" + self._func + "$" + lab + "\nD;JEQ"
            ...
            self._flag = False 
            Parser._error("if-goto",n, "Nedefinirana oznaka.")
    ...
    def goto(self, lab, n):
        if lab in self.labels: 
            ...
            self._flag = False
            Parser._error("goto", n, "Nedefinirana oznaka.")
            return ""
